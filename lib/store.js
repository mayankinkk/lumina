...
function saveToStorage(key, value) {
  ...
  try {
    ...
    if (e.name === "QuotaExceededError" || e.code === 22) {
      console.error("Storage full:", e.message);
    } else {
...
const useStore = create((set, get) => ({
  ...
  hydrate: async () => {
    ...
    set({ books, vocabulary, notes, highlights, readingSessions, fileCache, _hydrated: true });
  },
  ...
  removeBook: (id) => {
    ...
    saveToStorage(STORAGE_KEYS.books, updated);    const cache = { ...get().fileCache };
    delete cache[id];
  ...
  updateBook: (id, updates) => {
    ...
    saveToStorage(STORAGE_KEYS.books, updated);    set({ books: updated });
  },
