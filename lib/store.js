...
const useStore = create((set, get) => ({
  ...
  readerPresets: [],
  readingTimer: { enabled: false, duration: 25, breakDuration: 5, remaining: 1500, isBreak: false },
  bookOrientation: {},
  ...
  hydrate: async () => {
    ...
    const readingTimer = loadFromStorage("lumina_reading_timer", { enabled: false, duration: 25, breakDuration: 5, remaining: 1500, isBreak: false });
    ...
    set({ books, vocabulary, notes, highlights, readingSessions, bookmarks, readingQueue, dailyGoal, weeklyGoal, goalMode, customThemeColors, autoHideToolbar, tapZones, readerPresets, readingTimer, fileCache, _hydrated: true });
  },
  ...
  setReadingTimer: (config) => {
    const next = { ...get().readingTimer, ...config };
    saveToStorage("lumina_reading_timer", next);
    set({ readingTimer: next });
  },

  setBookOrientation: (bookId, mode) => {
    const bookOrientation = { ...get().bookOrientation, [bookId]: mode };
    saveToStorage("lumina_book_orientation", bookOrientation);
    set({ bookOrientation });
  },
